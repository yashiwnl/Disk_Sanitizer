...
def duplicate_files_scanner(directory_path):
  border = "-"*40
  timestamp = time.ctime
  log_file_name = "Duplicate_Files %s.log"%timestamp()
  log_file_name = log_file_name.replace(" ", "_")
  log_file_name = log_file_name.replace(":", "_")
...
def main():
  ...
  if len(sys.argv) == 2:
    if sys.argv[1] == "--h" or sys.argv[1] == "--H":
       ...
       print("python file_name.py directoryName")
       print("Directory name should be absolute path")

    else:
     schedule.every(1).minute.do(empty_files_scanner, sys.argv[1])
     schedule.every(1).minute.do(duplicate_files_scanner,sys.argv[1])
